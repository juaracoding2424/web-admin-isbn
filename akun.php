							...
							<table class="table table-row-dashed table-hover no-wrap fs-8 gy-5" id="example" style="width:100%">
								<thead>
									<tr class="text-start text-gray-500 fw-bold fs-8 text-uppercase gs-0">
                                        ...
                                        <th class="min-w-175px">Hak Akses</th>
                                        <th class="min-w-200px">Terakhir Login</th>
...
<script>
	...
	var populateDataSet = function(numb){
		...
		for( var i = 1; i<=numb; i++ ){
            let nama = RandomName();
            let username = nama.split(" ").join("") + getRandom(123,450);
            let email = nama.split(" ").join("") + "@mail.com";
			dataSetPop.push([
				i.toString(),
				nama,
                username,
				email,
                '<select class="form-select fs-7 select-costum" id="changeStatus_'+(i-1)+'" onChange="changeStatus('+(i-1)+')"><option value"">--Pilih status--</option><option value="aktif">Aktif</option><option value="blokir">Disable</option></select>', 
                '<select class="form-select fs-7 select-costum" id="changeAccess_'+(i-1)+'" onChange="changeAccess('+(i-1)+')"><option value"">--Pilih status--</option><option value="administrator">Administrator</option><option value="validator">Validator</option></select>', 
                randomDate(new Date(2024, 5, 1), new Date()),
				'<a class="badge badge-info h-30px m-1" href="detail_akun.php">Ubah</a>',
			]);
		}
		return dataSetPop;
	};
